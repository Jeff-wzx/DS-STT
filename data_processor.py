...
# Baidu ApolloScape data format:
    ...
    # pra_file_path中的pra=practice，执行文件所在的文件夹
            feature: (T, V ,C) 
                    C is the dimension of features, x,y label #**2
                    T is the temporal length of the data. history_frames + future_frames   #**6+6=12
                    V is the maximum number of objects. zero-padding for less objects. # 最大的目标数量115还是120，不够补零
'''
...
# 连续帧之间的步长，选取的特征id、最大的目标数量，邻居距离，总共12帧
frame_step=1
...
# file_path_list指向包含数据的文件的文件路径列表。该函数处理这些文件并创建存储在列表中的数据集。
# 然后再将生成的数据集保存在packle文件中
def GenerateData(file_path_list, data_root, is_train=True):
    all_data = []
    # max_object=[]
    # tqdm是一个快速，可扩展的Python进度条,展示进度条，应该是加载进度条，
    # 一个可视化的进度条，告诉你还有多少项需要处理，预计剩余时间，以及当前的处理速度。
    for file_path_idx in tqdm(file_path_list):
        # print(file_path_idx)
        # 读取变量，指定的文件的内容。它假定文件包含文本行，其中每行由空格分隔的值组成。
        # strip()从行中删除字符串两侧（前导和尾随）的空格字符（或者换行、制表符）。
        # split(' ')使用空格（' '）作为分隔符将行拆分；strip根据空格将行拆分为子字符串列表
        # strip删除空格和回车符号
        # 打开一个文件，逐行读取其内容，根据空格将每行拆分为子字符串，并将结果数据转换为浮点数的NumPy数组。
        with open(file_path_idx, 'r') as reader:
            content = np.array([x.strip().split(' ') for x in reader.readlines()]).astype(float)
        # 从数据中提取帧号。帧号位于数据集的第一列中
        # 查找数组中的唯一帧号。生成的唯一值，按升序排序。最后将排序后的唯一帧号转换为列表
        # np.unique() 函数去除其中的重复元素 ，并将元素由小到大排序，返回一个新的无重复元素的元组或者列表。
        # 因为收集数据时是按照小帧到大帧收集排序的，且数据集中，一帧有很多个目标，在数据集中有重复的帧号，通过这样处理得到唯一帧号

        scene_frames = content[:, 0].astype(np.int64)        
        unique_frames = sorted(np.unique(scene_frames).tolist())
        # 找起始帧
        # 根据数据集是用于训练还是测试（或），选择不同的帧来创建样本。该变量将存储样本的起始帧
        # 用于获取包含除最后一个索引之外的所有帧索引的子列表
        ...
        if is_train:
            ...
            # 这意味着在帧号中因为由唯一标识，第六个倍数的帧，取间隔六帧的数据，每第六帧作为创建数据样本的起始帧
            # 从零开始，每过六个提取一次，[0,6,12，18...]
            # 完整是这个样子[start:stop:step]
            start_frame_ids = unique_frames[::history_frames]
        data_list = []
        # 看每个txt数据集文件中有多少帧，其中训练的起始帧为开始到倒数十一帧
        for start_index in start_frame_ids:
            ...
            for frame_idx, frame in enumerate(sample_frames):
                # （exist_object_idx为索引值0，1，2，3，4，5，6...）
                exist_object_idx = []
