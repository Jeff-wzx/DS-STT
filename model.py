...
# 输入通道数6，输出通道数2，,模型的维度32，多头注意力的头有8个，
# Dropout 是一种正则化技术，它在训练期间将输入单元的一部分随机设置为 0，以防止过度拟合。dropout可以帮助纠正过拟合
# 前馈层的隐藏大小与输入/输出大小的比率=1。它确定输入的中间表示的维度。（指定前馈层尺寸与模型尺寸之比，为1）

class s2tnet(nn.Module):
    def __init__(self, in_chans=6, output_chans=2, d_model=32, nhead=8, feedforward_ratio=1, drop_rate=0.1):
        super().__init__()
        # 使用上述的参数进行初始化，进入后面的函数中进行调用
        ...
        """
              空间自注意力子层（到时候去掉看看）
              STLayer:空时transformer里面包括空间自注意力子层和时间卷积子层，两个层看看到时候怎么弄
        """
    ...
    def forward(self, batch,current_frame,device,
        ...
        """
        # encoder 1,ssa 空间自注意力子层（到时候去掉看看）
        """
